...
生产/auto 后端启动后会先渲染主界面，用户可以立即选择摄像头或视频文件；模型由独立线程按“环境与模型资产检查 → YOLO/ByteTrack → RTMPose → OSNet-AIN → GaitGraph2 → Dummy warmup”的顺序逐步准备，进度条和阶段文字显示在“输入源”区域下方。模型未完成预热前不会启用“开始”采集，因此不会阻塞 Tk 主线程或把半初始化模型用于正式图库。RTX 5060 实测生产完整冷启动约需 8～15 秒；完成预热后，单帧耗时取决于分辨率和同框人数。`demo` 诊断后端不加载生产模型，会直接进入界面。
